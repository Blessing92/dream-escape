...
export const getToday = (options: GetTodayOptions = {}) => {
  ...
  // This is necessary to compare with created_at from Supabase, because it's not at 0.0.0.0, so we need to set
  // the date to be END of the day when we compare it with earlier dates
  if (options?.end)
